  ...
  my.Deferred.when = function() {
    ...
    for (var i=0, _len=valsOPros.length; i < _len; i++) {
      if (isPromise(valsOPros[i])) {
        /* Si es una promesa, tenemos que esperar a que se resuelva */
        valsOPros[i].then((function(index) {
          return function() {
            /* Guardo el array con todos sus argumentos */
            resultados[index] = [].slice.call(arguments);
            resueltos++;
            /* ¿Hemos terminado? */
        ...
        /* Si es un valor, podemos meterlo directamente */
        resultados[i] = [valsOPros[i]];
        resueltos++;
      }
    }
